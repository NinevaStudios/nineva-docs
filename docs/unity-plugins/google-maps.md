...
### Using the plugin along with other native plugins
...
Just import the Play Services Resolver (Now called Google Dependency Manager) and it should resolve all the dependencies correctly. The plugin contains the `GoogleMapsDependencies.xml` file where its dependencies are declared.
